...
находясь перед некоторым кустом заданной во входном файле гряд
"""
from random import randint
summa_berries = 0
N = int(input("количество кустов: "))
beds = [randint(0, 10) for i in range(N)]
for i in range(len(beds) - 1):
    if beds[i - 1] + beds[i] + beds[i + 1] > summa_berries:
        summa_berries = beds[i - 1] + beds[i] + beds[i + 1]
